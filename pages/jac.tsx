...
const buildDraftText = (
  ...
  const accommodationsText =
    mode === 'selected' ? selectedText || allAccommodations : allAccommodations;
...
export default function JacTrial() {
  ...
  const [safetyGateAnswers, setSafetyGateAnswers] = useState<Record<string, string>>({});
  ...
  const safetyGate = assessmentProcess?.safetyGate || null;
  const isStrictSafetyGate = safetyGate?.mode === 'strict';
  const safetyGateQuestionKeys = useMemo(() => {
    if (!safetyGate) return [];
    return (safetyGate.followUpQuestions || []).map((_, index) => `safety_gate_${index}`);
  }, [safetyGate]);
  const isSafetyGateReady = useMemo(() => {
    if (!isStrictSafetyGate) return true;
    if (safetyGateQuestionKeys.length === 0) return false;
    return safetyGateQuestionKeys.every((key) => (safetyGateAnswers[key] || '').trim().length >= 6);
  }, [isStrictSafetyGate, safetyGateAnswers, safetyGateQuestionKeys]);
  const safetyGateAnsweredCount = useMemo(() => {
    return safetyGateQuestionKeys.filter((key) => (safetyGateAnswers[key] || '').trim().length >= 6)
      .length;
  }, [safetyGateAnswers, safetyGateQuestionKeys]);
  ...
  const buildApiUrlWithToken = useCallback(
    (path: string) => {
      const token = normalizeAccessToken(accessToken);
  ...
  useEffect(() => {
    ...
    setSafetyGateAnswers({});
    setRefining(false);
    setRefineMessage(null);
  }, [consultation, selectedTags, followUpAnswers]);

  useEffect(() => {
    if (safetyGateQuestionKeys.length === 0) {
      setSafetyGateAnswers({});
      return;
    }
    setSafetyGateAnswers((prev) => {
      const next: Record<string, string> = {};
      safetyGateQuestionKeys.forEach((key) => {
        next[key] = prev[key] || '';
      });
      return next;
    });
  }, [safetyGateQuestionKeys]);
    ...
    setSelectedTags((prev) => {
      ...
      (Object.keys(tagSuggestion.suggestions) as TagGroupKey[]).forEach((group) => {
        const combined = new Set([
          ...next[group],
          ...tagSuggestion.suggestions[group].map((item) => item.tag),
        ]);
  ...
  const fetchTagSuggestion = useCallback(
    async (force = false) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'タグ提案の取得中にエラーが発生しました。';
  ...
  const generateAssessment = async () => {
    const safetyGateFollowUps: FollowUpAnswer[] = (safetyGate?.followUpQuestions || [])
      .map((question, index) => {
        const key = `safety_gate_${index}`;
        const value = (safetyGateAnswers[key] || '').trim();
        return {
          key,
          label: `安全確認: ${question}`,
          value,
    ...
    const combinedFollowUpAnswers = [...followUpAnswers, ...safetyGateFollowUps];
    ...
    try {
      ...
      const fastResponse = await fetch(fastUrl, {
        ...
        body: JSON.stringify({
          consultation,
          selectedTags,
          followUpAnswers: combinedFollowUpAnswers,
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4 space-y-3 min-w-0">
                  ...
                  {!assessmentProcess && (
                    <p className="text-xs text-gray-500">
                      見立て生成後に、ソース選択と実行ステップの進捗を表示します。
                    </p>
                  )}
                  {assessmentProcess && (
                    <div className="space-y-3">
                      {assessmentProcess.safetyGate && (
                        <div
                          className={`rounded-lg border p-3 min-w-0 ${
                            assessmentProcess.safetyGate.mode === 'strict'
                              ? 'border-rose-200 bg-rose-50'
                              : assessmentProcess.safetyGate.mode === 'caution'
                        ...
                        >
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <p className="text-[11px] font-semibold text-gray-900">
                              安全ゲート: {assessmentProcess.safetyGate.mode.toUpperCase()}
                            </p>
                            <p className="text-[10px] text-gray-700">
                              policy: {assessmentProcess.safetyGate.recommendationPolicy}
                            </p>
                          </div>
                          <p className="text-[11px] text-gray-700 mt-1 break-words">
                            {assessmentProcess.safetyGate.summary}
                          </p>
                          {assessmentProcess.safetyGate.reasonCodes.length > 0 && (
                          ...
                          <div className="space-y-2">
                            {assessmentProcess.evidencePreview.slice(0, 3).map((item) => (
                              <div
                                ...
                                className="border border-gray-100 rounded-md px-2 py-1 min-w-0"
                              >
                    ...
                    <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 space-y-3">
                      <div>
                        ...
                        <p className="text-xs text-rose-800 mt-1">
                          安全ゲートが strict のため、以下の確認が揃うまで合意文書へ進めません。 (
                          {safetyGateAnsweredCount}/{safetyGateQuestionKeys.length})
                        </p>
                      </div>
                      {safetyGate.followUpQuestions.map((question, index) => {
                        const key = `safety_gate_${index}`;
                        return (
                          <label key={key} className="block">
                            ...
                            <textarea
                              className="mt-2 w-full rounded-xl border border-rose-200 bg-white px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-rose-200 min-h-[90px]"
                              placeholder="現場条件・法域・試行結果を具体的に入力"
                              value={safetyGateAnswers[key] || ''}
                              onChange={(event) =>
                                setSafetyGateAnswers((prev) => ({
                                  ...prev,
                          ...
                          <div className="grid gap-2">
                            {[
                              ...
                              <div key={lane.label} className="space-y-1">
                                ...
                                {index < 3 && (
                                  <div className="flex justify-center text-indigo-300 leading-none text-sm">
                                    ↓
                                  </div>
                                )}
                              </div>
                ...
                <div className="flex justify-between">
                  ...
                  <div className="text-right">
                    {isStrictSafetyGate && !isSafetyGateReady && (
                      <p className="text-[11px] text-rose-700 mb-1">
                        strictモード: 必須の追加確認を入力すると次へ進めます。
                      </p>
                    )}
                    <button
                      type="button"
                      onClick={async () => {
                        ...
                        if (ok && (!isStrictSafetyGate || isSafetyGateReady)) setStep(4);
                      }}
                      disabled={isStrictSafetyGate && !isSafetyGateReady}
        ...
        <section className="space-y-6 min-w-0">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                <p className="text-xs text-gray-500">安全ゲート</p>
                {assessmentProcess?.safetyGate ? (
                  <div className="mt-1 space-y-1">
                    <p className="text-sm font-semibold text-gray-900">
                      {assessmentProcess.safetyGate.mode.toUpperCase()} /{' '}
                      {assessmentProcess.safetyGate.recommendationPolicy}
                    </p>
