...
describe('Falcon next NBL static site candidate', () => {
  ...
  it('frames the 21-view surface as both a guidebook and a reference toolkit', () => {
    ...
    expect(screen.getAllByText('使う先、見る面、手元の問いを分ける。').length).toBeGreaterThan(0);
    expect(screen.getByText('このページの読み方')).toBeInTheDocument();
    expect(screen.getByText('使う先 / 4つの実装領域')).toBeInTheDocument();
    expect(screen.getByText('見る面 / 3つの設計面')).toBeInTheDocument();
    expect(screen.getByText('手元の問い / 21視点')).toBeInTheDocument();
    ...
    expect(screen.getByText('3つの設計面から、21の問いを開く。')).toBeInTheDocument();
    expect(screen.getByText('上部で選ぶ4つの実装領域は、21視点を持ち込む相手先です。このボードでは、どの領域にも使える問いを、健康時間、入口・翻訳、職場・価値の3面から開きます。')).toBeInTheDocument();
    expect(screen.getByText('一文から開く例')).toBeInTheDocument();
    expect(screen.getByText('復職後、短時間勤務なのに評価が曖昧で不安が強い。')).toBeInTheDocument();
    expect(screen.getByText('まず3面へ分ける')).toBeInTheDocument();
    expect(screen.getByText('必要な観測点へ進む')).toBeInTheDocument();
