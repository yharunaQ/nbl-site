...
describe('Axiom next NBL public candidate final QA', () => {
  ...
  it('covers all 9 public-candidate pages and the main visual-heavy pages', () => {
    ...
    expect(
      run.visualQaMatrix.filter((item) => item.pageSlug === 'work-design-views-guide'),
    ).toHaveLength(22);
  });
  ...
  it('builds a valid NBL report article-to-infographic QA map for all 37 articles', () => {
  ...
  it('renders the final QA page as a readable internal review surface', () => {
    ...
    expect(screen.getByText('NBLレポート37記事・図解対応表')).toBeInTheDocument();
    expect(
      screen.getByText('axiom_nbl_report_article_visual_qa_valid', { exact: false }),
    ).toBeInTheDocument();
    expect(container.querySelectorAll('[data-report-article-visual-qa-item]')).toHaveLength(37);
    expect(screen.getByText(/8つの課題の地図/)).toBeInTheDocument();
